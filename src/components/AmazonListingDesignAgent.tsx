...
export default function AmazonListingDesignAgent({ onShowPricing }: AmazonListingDesignAgentProps) {
    ...
    const generateDesignPlan = async () => {
        ...
        try {
            ...
            const planText = currentPlan.toLowerCase();
            const isFree = planText.includes('free') && !planText.includes('starter') && !planText.includes('professional') && !planText.includes('agency');
            const imageCount = isFree ? 3 : 7;

            console.log(`[DEBUG] Plan: ${currentPlan}, isFree: ${isFree}, imageCount: ${imageCount}`);

            console.log(`Generating plan for: ${currentPlan} (isFree: ${isFree}, imageCount: ${imageCount})`);
...
CRITICAL REQUIREMENT: Your analysis MUST prepare for exactly ${imageCount} listing images.
...
Respond with ONLY a JSON object analyzing the project:
{
    ...
    "designStrategy": "Overall strategy for the ${currentPlan.toLowerCase().includes('free') ? '3' : '7'}-image listing"
            ...
            const analysisResponse = await fetch('/api/analyze', {
                ...
                body: JSON.stringify({
                    max_tokens: 4000,
                    system: `You are an expert Amazon listing strategist. You ONLY respond with valid JSON as requested. Do not include any conversational text before or after the JSON. 
                    
                    CRITICAL RESTRICTION: You must NEVER use the following restricted keywords: ${restrictedKeywords.join(', ')}`,
                    messages: [{
                        role: 'user',
            ...
            try {
                ...
                console.error("Analysis Parsing Failed:", analysisText);
                throw new Error("Failed to parse AI analysis. The model might have encountered an error or safety filter.");
            }
...
Respond with ONLY a JSON object:
{
    "images": [
        ...
        ... (repeat for all ${imageCount} images)
    ]
}

CRITICAL: You must provide EXACTLY ${imageCount} items in the "images" array. Do not stop early. Do not summarize. Provide full detail for every image.`;
            ...
            const designResponse = await fetch('/api/analyze', {
                ...
                body: JSON.stringify({
                    max_tokens: 4000,
                    system: `You are a professional Amazon Design Agent. You MUST return a JSON object with exactly ${imageCount} image plans. 
                    ...
                    CRITICAL: Do not be lazy. You MUST provide all ${imageCount} images in full detail.
                    RESTRICTED KEYWORDS: ${restrictedKeywords.join(', ')}`,
                    messages: [{
                        role: 'user',
            ...
            let designPlanResult;
            try {
                designPlanResult = extractJson(designText);
            } catch (e) {
                console.error("Design Plan Parsing Failed:", designText);
                throw new Error("Failed to parse AI design plan. The response was too large or malformed.");
            }
